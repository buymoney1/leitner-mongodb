...
export default function VocabularyList({ vocabularies }: VocabularyListProps) {
  ...
  return (
    <div className="min-h-full bg-gradient-to-br from-gray-900 via-black to-gray-900 p-4">
      ...
      <div className="relative mb-6 group">
        ...
        <div className="relative bg-gray-900/80 rounded-2xl border border-gray-700/50 backdrop-blur-xl">
          <Search className="absolute right-4 top-1/2 transform -translate-y-1/2 text-cyan-400 w-5 h-5" />
          <input
            type="text"
            placeholder="جستجوی لغت یا معنی..."
            value={searchTerm}
            onChange={(e) => setSearchTerm(e.target.value)}
            className="w-full p-4 pr-12 bg-transparent text-white placeholder-gray-400 focus:outline-none rounded-2xl"
          />
        </div>
      </div>
      ...
      <div className="space-y-3">
        {filteredVocabularies.length > 0 ? (
            ...
            <div
              ...
              className={`px-3 pt-2 pb-1 rounded-2xl border backdrop-blur-sm transition-all duration-300 cursor-pointer group relative overflow-hidden
                ${selectedWord?.id === vocab.id
                  ...
                  : 'bg-gray-800/30 border-gray-700/50 hover:bg-gray-800/50 hover:border-gray-600/50 hover:shadow-lg'
                }`}
            >
          ...
          <div className="text-center py-12">
            <div className="p-4 bg-gradient-to-r from-gray-800/50 to-gray-900/50 rounded-2xl border border-gray-700/50 backdrop-blur-sm">
              <BookOpen className="w-12 h-12 text-gray-600 mx-auto mb-4" />
              <p className="text-gray-500 text-lg mb-2">لغتی یافت نشد</p>
      ...
      {selectedWord && (
        ...
        >
          <div 
            className="bg-gradient-to-br from-gray-900 to-black rounded-3xl border border-cyan-500/30 p-6 max-w-md w-full mx-auto shadow-2xl shadow-cyan-500/20"
          ...
          >
            <div className="text-center mb-6">
              ...
              <h3 className="text-2xl font-bold text-cyan-300 mb-2">{selectedWord.word}</h3>
              <p className="text-gray-300 text-lg">{selectedWord.meaning}</p>
            </div>
            <button
              onClick={() => setSelectedWord(null)}
